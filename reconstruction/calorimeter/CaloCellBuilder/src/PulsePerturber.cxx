 ...
 * or noisy cells (by adding extra gaussian noise) based on configuration
 * properties and event numbers. This is useful for testing the robustness of
 * reconstruction algorithms against detector defects.
 *
 * Properties:
 * - DeadModules: List of booleans indicating if a block of events/cells is
 * dead.
 ...
 */
PulsePerturber::PulsePerturber(std::string name)
    : IMsgService(name), AlgTool(), m_rng(0) {
...
PulsePerturber::~PulsePerturber() { ; }

//!=====================================================================

StatusCode PulsePerturber::initialize() {
...
StatusCode PulsePerturber::finalize() { return StatusCode::SUCCESS; }
 ...
 */
StatusCode PulsePerturber::execute(SG::EventContext &ctx,
                                   Gaugi::EDM *edm) const {
  ...
  if (!event.isValid()) {
    MSG_FATAL("It's not possible to read the xAOD::EventInfoContainer from "
              "this Context");
  }
  // get the old ones
  ...
  for (std::size_t block = 0; block < m_eventNumberRange.size(); ++block) {
    ...
    if ((eventNumber >= m_eventNumberRange[block][0]) &&
        (eventNumber <= m_eventNumberRange[block][1])) {
      // MSG_INFO("event number "<<eventNumber<<" is in the range
      // "<<m_eventNumberRange[block][0]<<" - "<<m_eventNumberRange[block][1]);
      unsigned long int id =
          m_deadModules[block]
              ? static_cast<unsigned long int>(cell->hash() / 1e7)
              : cell->hash();
      bool found = std::find(m_cells[block].begin(), m_cells[block].end(),
                             id) != m_cells[block].end();
      // MSG_INFO("Is a dead module anomaly? "<< (m_deadModules[block] ?
      // "yes":"no") ); MSG_INFO("checking cell with hash id: "<< id <<" (
      // found? " << (found?"yes":"no") <<" )");
      if (found) {
        ...
        if (m_deadModules[block]) {
          ...
          AddGaussianNoise(pulse_sum, m_noiseMean,
                           m_noiseStdFactor[block] * m_noiseStd);
          MSG_INFO("pulse sum after noise: " << pulse_sum);
          MSG_INFO("cell energy after noise: " << cell->e());
        }
...
void PulsePerturber::AddGaussianNoise(std::vector<float> &pulse,
                                      float noiseMean, float noiseStd) const {
