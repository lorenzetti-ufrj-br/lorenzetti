...
basepath         = os.getcwd()
input_path       = f"{basepath}/mc25_13TeV.20251215.physics_Main.electron.500k.jobs.json"
...
with Flow(name="mc25_13TeV.20251215.physics_Main.electron.500k", path=f"{basepath}/mc25_13TeV.20251215.physics_Main.electron.500k") as session:


    input_dataset    = Dataset(name="mc25_13TeV.20251215.physics_Main.electron.500k.jobs.json", path=input_path)
    image            = Image(name="lorenzetti", path=image_path)
    partitions       = 'cpu'

    pre_exec = f"source {repo_build_path}/lzt_setup.sh"

    command = f"{pre_exec} && gen_single.py -o %OUT --job-file %IN -m -p Electron"

    task_1 = Task(name="mc25_13TeV.20251215.physics_Main.electron.500k.EVT",
                  image=image,
                  command=command,
                  input_data=input_dataset,
                  outputs={'OUT':'electron.EVT.root'},
                  partition=partitions,
                  binds=binds)
    
    
    command = f"{pre_exec} && simu_trf.py -i %IN -o %OUT -nt 40"
    task_2 = Task(name="mc25_13TeV.20251215.physics_Main.electron.500k.HIT",
                  image=image,
                  command=command,
                  input_data=task_1.output('OUT'),
                  outputs= {'OUT':'electron.HIT.root'},
                  partition=partitions,
                  binds=binds)
    
    
    command = f"{pre_exec} && digit_trf.py -i %IN -o %OUT -nt 10 --events-per-job 100 -m"
    task_3 = Task(name="mc25_13TeV.20251215.physics_Main.electron.500k.ESD",
                  image=image,
                  command=command,
                  input_data=task_2.output('OUT'),
                  outputs= {'OUT':'electron.ESD.root'},
                  partition=partitions,
                  binds=binds)
    
    command = f"{pre_exec} && reco_trf.py -i %IN -o %OUT -nt 10 --events-per-job 100 -m"
    task_4 = Task(name="mc25_13TeV.20251215.physics_Main.electron.500k.AOD",
                  image=image,
                  command=command,
                  input_data=task_3.output('OUT'),
                  outputs= {'OUT':'electron.AOD.root'},
                  partition=partitions,
                  binds=binds)
    
    command = f"{pre_exec} && ntuple_trf.py -i %IN -o %OUT -nt 10 --events-per-job 100 -m"
    task_5 = Task(name="mc25_13TeV.20251215.physics_Main.electron.500k.NTUPLE",
                  image=image,
                  command=command,
                  input_data=task_4.output('OUT'),
                  outputs= {'OUT':'electron.NTUPLE.root'},
                  partition=partitions,
                  binds=binds)
